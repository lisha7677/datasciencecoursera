...
```
#### remove words with count 1 from corpus
```{r}
...
biModel<-function(x) {
        biTop4<-cbind.data.frame(value1=biNo1, value2=biNo2, value3=biNo3, value4=biNo4)
        predict<-biTop4[names(biNo1)==x,]
        predict
}
```
#### build trigram model
...
tri1st<-sapply(strsplit(triN2$ngrams, ' '), function(x) {paste(x[1],x[2])})
tri2nd<-sapply(strsplit(triN2$ngrams, ' '), function(x) {x[3]})
triNo1<-tapply(tri2nd, tri1st, function(x) {x[1]})
triNo2<-tapply(tri2nd, tri1st, function(x) {x[2]})
triNo3<-tapply(tri2nd, tri1st, function(x) {x[3]})
triNo4<-tapply(tri2nd, tri1st, function(x) {x[4]})
triModel<-function(x) {
        triTop4<-cbind.data.frame(value1=triNo1, value2=triNo2, value3=triNo3, value4=triNo4)
        predict<-triTop4[names(triNo1)==x,]
        predict
}
```
...
quad1st<-sapply(strsplit(quadN2$ngrams, ' '), function(x) {paste(x[1],x[2],x[3])})
quad2nd<-sapply(strsplit(quadN2$ngrams, ' '), function(x) {x[4]})
quadNo1<-tapply(quad2nd, quad1st, function(x) {x[1]})
quadNo2<-tapply(quad2nd, quad1st, function(x) {x[2]})
quadNo3<-tapply(quad2nd, quad1st, function(x) {x[3]})
quadNo4<-tapply(quad2nd, quad1st, function(x) {x[4]})
quadModel<-function(x) {
        quadTop4<-cbind.data.frame(value1=quadNo1, value2=quadNo2, value3=quadNo3, value4=quadNo4)
        predict<-quadTop4[names(quadNo1)==x,]
        predict
}
```
